...
def S_mat(atoms , geom):
	...
	for i in range(nbasis):
		for j in range(nbasis):
			overlap_int_matrix[i][j] = S(orbitals[i],orbitals[j])
...
def T_mat(atoms , geom):
	...
	for i in range(nbasis):
		for j in range(nbasis):
			Kineic_matrix[i][j] = T(orbitals[i],orbitals[j])
...
def Eri_mat(atoms , geom, S_matrix):
	...
	for i in range(len(unique_indices_list)):
		...
		temp_eri_1D.append([orbitals.index(unique_indices_list[i][0])]+[orbitals.index(unique_indices_list[i][1])]+[orbitals.index(unique_indices_list[i][2])]+[orbitals.index(unique_indices_list[i][3])] +[eri_temp])
